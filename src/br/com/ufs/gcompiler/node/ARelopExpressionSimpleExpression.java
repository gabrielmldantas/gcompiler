/* This file was generated by SableCC (http://www.sablecc.org/). */

package br.com.ufs.gcompiler.node;

import br.com.ufs.gcompiler.analysis.*;

@SuppressWarnings("nls")
public final class ARelopExpressionSimpleExpression extends PSimpleExpression
{
    private PAdditiveExpression _left_;
    private PRelop _relop_;
    private PAdditiveExpression _right_;

    public ARelopExpressionSimpleExpression()
    {
        // Constructor
    }

    public ARelopExpressionSimpleExpression(
        @SuppressWarnings("hiding") PAdditiveExpression _left_,
        @SuppressWarnings("hiding") PRelop _relop_,
        @SuppressWarnings("hiding") PAdditiveExpression _right_)
    {
        // Constructor
        setLeft(_left_);

        setRelop(_relop_);

        setRight(_right_);

    }

    @Override
    public Object clone()
    {
        return new ARelopExpressionSimpleExpression(
            cloneNode(this._left_),
            cloneNode(this._relop_),
            cloneNode(this._right_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseARelopExpressionSimpleExpression(this);
    }

    public PAdditiveExpression getLeft()
    {
        return this._left_;
    }

    public void setLeft(PAdditiveExpression node)
    {
        if(this._left_ != null)
        {
            this._left_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._left_ = node;
    }

    public PRelop getRelop()
    {
        return this._relop_;
    }

    public void setRelop(PRelop node)
    {
        if(this._relop_ != null)
        {
            this._relop_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._relop_ = node;
    }

    public PAdditiveExpression getRight()
    {
        return this._right_;
    }

    public void setRight(PAdditiveExpression node)
    {
        if(this._right_ != null)
        {
            this._right_.parent(null);
        }

        if(node != null)
        {
            if(node.parent() != null)
            {
                node.parent().removeChild(node);
            }

            node.parent(this);
        }

        this._right_ = node;
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._left_)
            + toString(this._relop_)
            + toString(this._right_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._left_ == child)
        {
            this._left_ = null;
            return;
        }

        if(this._relop_ == child)
        {
            this._relop_ = null;
            return;
        }

        if(this._right_ == child)
        {
            this._right_ = null;
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        if(this._left_ == oldChild)
        {
            setLeft((PAdditiveExpression) newChild);
            return;
        }

        if(this._relop_ == oldChild)
        {
            setRelop((PRelop) newChild);
            return;
        }

        if(this._right_ == oldChild)
        {
            setRight((PAdditiveExpression) newChild);
            return;
        }

        throw new RuntimeException("Not a child.");
    }
}
